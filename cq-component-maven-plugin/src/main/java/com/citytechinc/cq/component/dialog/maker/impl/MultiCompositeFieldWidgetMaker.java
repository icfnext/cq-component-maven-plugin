...
public class MultiCompositeFieldWidgetMaker extends AbstractWidgetMaker{
	...
	public DialogElement make(String xtype, Field widgetField,
		...
		MultiCompositeField multiCompositeFieldAnnotation = (MultiCompositeField) ctWidgetField.getAnnotation(MultiCompositeField.class);
		...
		List<DialogElement> widgetCollection=buildWidgetCollection(ctContainingClass, ctWidgetField, widgetField, xtypeMap, xTypeToWidgetMakerMap, classLoader, classPool);

		return new MultiCompositeFieldWidget(matchBaseName, prefix, fieldLabel, fieldDescription, !isRequired, hideLabel,defaultValue, name, fieldName, additionalProperties,widgetCollection);
	}

	private List<DialogElement> buildWidgetCollection(CtClass componentClass,CtField curField, Field trueField,Map<Class<?>, String> classToXTypeMap,Map<String, WidgetMaker> xTypeToWidgetMakerMap,ClassLoader classLoader, ClassPool classPool) throws InvalidComponentFieldException, ClassNotFoundException, CannotCompileException, NotFoundException, SecurityException, NoSuchFieldException {
