...
public class MultifieldWidgetMaker extends AbstractWidgetMaker {
	...
	public DialogElement make(String xtype, Field widgetField, CtField ctWidgetField, Class<?> containingClass,
		...
		return new MultiValueWidget(WidgetFactory.MULTIFIELD_XTYPE, name, fieldName, fieldLabel, fieldDescription,
			isRequired, hideLabel, defaultValue, additionalProperties, fieldConfig);

	}

	private static final String getInnerXTypeForMultiField(Field widgetField, DialogField fieldAnnotation,
		...
		/*
		 * Check to see if a field configuration annotation is present. If so,
		 * use that
		 */
		...
		/*
		 * Otherwise, see if we can derive the xtype from the parameterized
		 * field
		 */
