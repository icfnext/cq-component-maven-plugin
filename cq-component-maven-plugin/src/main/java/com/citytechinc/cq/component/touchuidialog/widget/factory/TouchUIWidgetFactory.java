...
public class TouchUIWidgetFactory {
    ...
    public static TouchUIDialogElement make(TouchUIWidgetMakerParameters widgetMakerParameters, int rankingCeiling) throws InvalidComponentFieldException, TouchUIDialogGenerationException {
        ...
        try {
            TouchUIWidgetMaker widgetMaker = context.getMakerClass().getConstructor(TouchUIWidgetMakerParameters.class).newInstance(widgetMakerParameters);
            return widgetMaker.make();
        } catch (Exception e) {
            throw new TouchUIDialogGenerationException("Exception encountered generating widget for " + widgetMakerParameters.getCtMember().getName() + " of " + widgetMakerParameters.getClass().getName(), e);
        }
    }

    public static TouchUIWidgetMakerContext getWidgetMakerForMemberParameters(TouchUIWidgetMakerParameters parameters, int rankingCeiling) throws InvalidComponentFieldException {
        TouchUIWidgetConfigHolder widgetConfig = getWidgetConfigForParameters(parameters, rankingCeiling);

        //If we were able to lookup a widget configuration with a valid maker class return it
        ...
        //If a widget configuration was found but no maker class was associated with it, use the default class
        ...
        //If we have gotten here this means there is probably not a stacked annotation an we should attempt to guess the appropriate field type
        String inferredResourceType = getResourceTypeForMemberParameters(parameters);

        if (StringUtils.isBlank(inferredResourceType)) {
            throw new InvalidComponentFieldException("A resource type could not be determined for " + parameters.getCtMember().getName() + " of " + parameters.getClass().getName());
        }

        TouchUIWidgetConfigHolder inferredWidgetConfig = getWidgetConfigForResourceType(inferredResourceType, parameters, rankingCeiling);
     ...
     * annotation if found.
     *
     * @param parameters The parameters identifying the Component member being processed
     * @param rankingCeiling Widget configurations with rankings higher than the ceiling will not be considered
     * @return The determined widget configuration or null
     */
    @Nullable
    private static TouchUIWidgetConfigHolder getWidgetConfigForParameters(TouchUIWidgetMakerParameters parameters, int rankingCeiling) {
        ...
        for (Class<?> currentWidgetAnnotation : parameters.getWidgetRegistry().getRegisteredAnnotations()) {
            if (parameters.getCtMember().hasAnnotation(currentWidgetAnnotation)) {
                TouchUIWidgetConfigHolder potentialWidgetConfig = parameters.getWidgetRegistry().getWidgetForAnnotation(currentWidgetAnnotation);
                if (rankingCeiling < 0 || potentialWidgetConfig.getRanking() < rankingCeiling) {
                    if (highestRankedConfig == null || highestRankedConfig.getRanking() < potentialWidgetConfig.getRanking()) {
    ...
    @Nullable
    private static TouchUIWidgetConfigHolder getWidgetConfigForResourceType(String resourceType, TouchUIWidgetMakerParameters parameters, int rankingCeiling) {
        TouchUIWidgetConfigHolder highestRankedConfig = null;

        for (Class<?> currentWidgetAnnotation : parameters.getWidgetRegistry().getRegisteredAnnotations()) {
            TouchUIWidgetConfigHolder currentConfig = parameters.getWidgetRegistry().getWidgetForAnnotation(currentWidgetAnnotation);

            if (resourceType.equals(currentConfig.getResourceType()) && (rankingCeiling < 0 || currentConfig.getRanking() < rankingCeiling)) {
